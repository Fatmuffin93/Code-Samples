    ...
    public class ClimbingMovement : StateActions
    {
        ...
        void CheckRaycast(StateManager sm)
        {
            ...
            if (Physics.Raycast(frontOrigin, states.transform.forward, out front, 0.75f, Layers.ignoreLayersController, QueryTriggerInteraction.Ignore))
            {
                ...
                {
                    //Set variables for new climb and get ready to transfer to it
                    ignoreGravity = true;
                ...
                {
                    //Leave the climb if the front hits the ground
                    if (angle < 70)
                    {
                        states.rigid.velocity = Vector3.zero;
                        states.climbHit = front;
            ...
            else
            {
                ...
                {
                    //If nothign has hit then I'm in the air and I need to leave this climb
                    underHit = false;
                    states.rigid.velocity = Vector3.zero;
        ...
        void Rotate(StateManager sm)
        {
            if (cameraTransform.value == null)
                return;
            //keep track of previous rotation in case we need to revert back to it, in the the case of detecting an unclimbable surface
        ...
        bool safeTurn = true;
        //Once the movement and rotation values are set, then see if I can actually move that amount safely
        void SafeMove()
        {
            ...
            Vector3 targetVelocity = GetRotationVelocity(); //How fast I need to move backwards to rotate safekly along the climb 
            ...
            //Check if I can move safely after checking if I can rotate safely
            ...
            //Add artificial gravity to keep me more attached to the climb
            if (!ignoreGravity)
                states.rigid.velocity -= climbVel;
        }

        //Figure out how fast the turn is to figure out how fast the player needs to move to counteract the turn
        //It's essentially making it seem that the player is rotating on a pivot that isn't actually it's pivot
        ...
        //Transfers the player to the new climb while the player loses control for a short bit
        ...
        //Get the new normal when entering and completing a transfer
        void CheckUnder()
        {
            Vector3 underOrigin = states.transform.position;
            underOrigin += states.transform.forward * 1.7f + (states.transform.up * 0.5f);
