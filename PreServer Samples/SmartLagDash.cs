...
namespace PreServer
{
    ...
    public class SmartLagDash : StateActions
    {
        ...
        float rendTime = 0;
        bool debuglockPath = false; //Debug variable to lock the path in place
        int ghostIndex = 0;
        ...
        void GeneratePath(StateManager states, bool setTime)
        {
            ...
            //Generating a path on climbs is different than generating on the ground or air
            if (player.climbState == PlayerManager.ClimbState.CLIMBING)
            {
                CheckUnder();
            ...
            //Making sure the player doesn't go inside a wall based on the calculated path
        ...
        void CalculatePath(float remainingDistance, Vector3 up, Vector3 dir, Vector3 start, float remainingTime, Quaternion startRot)
        {
            ...
            //INCREASING SPHERE SIZE TO FULL RADIUS CAUSES THE SPHERECAST TO FAIL BECAUSE IT START TOUCHING THE GROUND
                ...
                else
                {
                    ...
                    if (Physics.SphereCast(start + up, 0.375f, dir, out hitInfo, remainingDistance, Layers.ignoreLayersController, QueryTriggerInteraction.Ignore))
                    {
                        ...
                        if (angle >= 46 || hitInfo.transform.tag == "Climb" || !player.isGrounded)
                        {
                            ...
                            //ERROR: This can cause you to go under the level
                            target = hitInfo.point - (dir * 2f) - (up.normalized * 0.375f);
                            path[path.Count - 1].time = (Vector3.Distance(start, target) / distance) * totalTime;
                            remainingDistance = 0;
                            remainingTime = 0;
                            //Debug.LogError("Getting pushed back");
                        }
                        else
                        {
            ...
            {
                Debug.LogError("Aborting Smart Dash Pathing Loop: Start and End positions are the same");
                return;
            }
        ...
        /// <summary>
        /// Used to change the normal given by the sphere cast because hitting a corner 
        /// retrieves a normal that is perpendicular to the sphere
        /// where as we want a normal to be perpendicular to the wall/ground
        /// </summary>
        ...
        void CalculatePathClimb(float remainingDistance, Vector3 up, Vector3 dir, Vector3 start, float remainingTime, Quaternion startRot)
        {
            ...
            if (Physics.SphereCast(start + up, 0.375f, dir, out triggerInfo, remainingDistance, Layers.ignoreLayersController, QueryTriggerInteraction.Collide))
            {
                ...
                {
                    //Under has been hit, figure out which one is closer and use that info first
            ...
            {
                //under has been hit but forward hasn't, use under
                if (underInfo.transform != null)
                {
                    CheckClimbHit(underInfo, start, tempPos, ref target, ref dir, ref up, ref remainingTime, ref remainingDistance);
                }
                ...
                {
                    //If underneath and forward aren't hit, then use up the rest of the distance because ain't nothing gonna stop us baby!!!!
                    //path[path.Count - 1].time = Mathf.Abs(remainingTime);
        ...
        private void CheckClimbTrigger(RaycastHit triggerInfo, RaycastHit climbHit, Vector3 start, Vector3 tempPos, ref Vector3 target, ref Vector3 dir, ref Vector3 up, ref float remainingTime, ref float remainingDistance)
        {
            ...
            else
            {
                ...
                if (Physics.SphereCast(start + up, 0.375f, dir, out hitInfo, remainingDistance, Layers.ignoreLayersController, QueryTriggerInteraction.Ignore))
                {
                    ...
                    {
                        //move the target point back the length of the squirrel and end the dash
                        //ERROR: This is also messing up corners
                        target = hitInfo.point - (dir * 2f) - (up.normalized * 0.375f);
                        path[path.Count - 1].time = (Vector3.Distance(start, target) / distance) * totalTime;
                        remainingDistance = 0;
                        remainingTime = 0;
                ...
                {
                    //if under hasn't been hit then use up remaining distance
        ...
        /// <summary>
        /// Check underneath the player in increments to see if the ground changes
        /// If there is a change in ground, then return the change
        /// </summary>
        /// <param name="distance">The remaining distance, gets decremented until it's all used up</param>
        /// <param name="start"></param>
        /// <param name="up"></param>
        /// <param name="dir"></param>
        /// <param name="temp">projected position of the squirrel if the ground changes or if remaining distance is used</param>
        /// <returns></returns>
        RaycastHit CheckUnder(float distance, Vector3 start, Vector3 up, Vector3 dir, out Vector3 temp)
        {
            ...
            while (tempRemaining > 0)
            {
                ...
                {
                    //Used to check corners if the straight down check fails
                    ...
                    {
                        //If all checks fail then use the previous registered hit
                        //If we allow the player to keep moving they would go off the wall into the air
        ...
        //Used to re-establish the ground when starting the dash on a climb
        void CheckUnder()
        {
        ...
        /// <summary>
        /// Gets a position on a line between 2 vectors
        /// </summary>
        /// <param name="p">The point being projected on the line</param>
        ...
        void SafeClimb()
        {
            ...
            //{
            //    backBlocked = CheckBackBlockedRaycast(originalDirection, hit, false, ref targetPos);
            //}
            ...
            if (!backBlocked)
            {
                ...
                //Once the squirrel's body is secured, move him back more if he is not safely on climbs
                if (!backBlocked)
                {
            ...
            //Keep rotating if that squirrel is not safely adjusted on a climb
            if (angle > -180f)
            {
                ...
                while (!climbHit)
                {
                    ...
                    if (angle <= -180)
                        break;
                    climbHit = CheckClimbHit(targetPos + (path[path.Count - 1].up * 0.1f) + (newDirection * 1.5f), hit);
                    if (Physics.SphereCast(targetPos + path[path.Count - 1].up * 0.4f, 0.375f, newDirection, out hit, 2f, Layers.ignoreLayersController, QueryTriggerInteraction.Ignore))
                    {
                        climbHit = false;
                    }
                }
        ...
        //Used for the climb safety net
        //Since I don't know which direction is the quicker to get the squirrel into a safe rotation I alternate
        ...
        public override void OnUpdate(StateManager states)
        {
            ...
            //Check to see if the button is held rather than just pressed
            ...
            {
                if (buttonHeld || debuglockPath)
                {
                    if (Input.GetKeyDown(KeyCode.M))
                    {
                        debuglockPath = !debuglockPath;
                    }
                    if (Input.GetKeyDown(KeyCode.Equals))
                    {
                        ghostIndex++;
                        if (ghostIndex >= path.Count)
                            ghostIndex = 0;
                    }
                    else if(Input.GetKeyDown(KeyCode.Minus))
                    {
                        ghostIndex--;
                        if (ghostIndex < 0)
                            ghostIndex = path.Count - 1;
                    }

                    //Slow down the game so the player has tiem to react and make adjustments
                    if (!timeScaleSet)
                    {
                    ...
                    {
                        //Increment the timescale overtime as the timer runs out
                    ...
                    //Keep checking the input to make sure the button is held
                    if (((Input.GetAxisRaw("DashCont") != 1f && !Input.GetButton("Dash")) || player.lagDashCooldown >= 1f) && !debuglockPath)
                    {
                        buttonHeld = false;
                        timeScaleSet = false;
                        player.lagDashCooldown = 1f;
                    ...
                    {
                        //The blue squirrel is used to check where the player will end up when using this power
                    ...
                    //If the player's position or rotation changes during the slow-mo, then recalculate the path
                    if ((player.transform.position != path[0].lerper.startVal || player.transform.rotation != prevRotation) && !debuglockPath)
                    {
                        if (blueSquirrel)
                            GetGhostTime();
                        while (path.Count > 0)
                    ...
                    //Update the blue squirrel's position and rotation
                    if (blueSquirrel)
                    {
                        ...
                        if (debuglockPath)
                        {
                            if (ghostIndex >= path.Count)
                                ghostIndex = 0;
                            blueSquirrel.transform.position = path[ghostIndex].lerper.endVal;
                            blueSquirrel.transform.rotation = path[ghostIndex].slerper.endVal;
                        }
                    }
                    if (bsRend != null)
                    {
                ...
                else
                {
                    ...
                    //Set variables for the actual dash and reset others
                    if (!timeScaleSet)
                    {
                        debuglockPath = false;
                        ghostIndex = 0;
                    ...
                    //Blue squirrel is no longer needed, goodbye new friend
                    if (blueSquirrel)
                    {
                        Destroy(bsTrail);
        ...
        //Prevent the player from rotating if more than half of the squirrel's body will be off the climb
        ...
        //Adjusting the camera if the player is climbing
        ...
        public override void OnExit(StateManager states)
        {
            ...
            for (int i = 0; i < path.Count; ++i)
            {
                ...
                Debug.DrawRay(path[i].lerper.endVal, path[i].up, Color.yellow, 5f);
            }
